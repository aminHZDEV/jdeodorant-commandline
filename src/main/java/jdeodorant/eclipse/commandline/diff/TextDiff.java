 ...
 * http://code.google.com/p/google-jdeodorant.eclipse.commandline.diff-match-patch/
 *
...
package jdeodorant.eclipse.commandline.diff;
...
/*
 * Functions for jdeodorant.eclipse.commandline.diff, match and patch.
...
/**
 * Class containing the jdeodorant.eclipse.commandline.diff, match and patch methods.
 * Also contains the behaviour settings.
 */
public class TextDiff {

  // Defaults.
  // Set these on your TextDiff instance to override the defaults.

  /**
   * Number of seconds to map a jdeodorant.eclipse.commandline.diff before giving up (0 for infinity).
   */
  ...
  /**
   * The data structure representing a jdeodorant.eclipse.commandline.diff is a Linked list of Diff objects:
   ...
   * Run a faster, slightly less optimal jdeodorant.eclipse.commandline.diff.
   ...
   *     line-level jdeodorant.eclipse.commandline.diff first to identify the changed areas.
   *     If true, then run a faster slightly less optimal jdeodorant.eclipse.commandline.diff.
   * @return Linked List of Diff objects.
   */
  public LinkedList<Diff> diff_main(String text1, String text2,
                                    boolean checklines) {
    // Set a deadline by which time the jdeodorant.eclipse.commandline.diff must be complete.
    long deadline;
    if (Diff_Timeout <= 0) {
   ...
   *     line-level jdeodorant.eclipse.commandline.diff first to identify the changed areas.
   *     If true, then run a faster slightly less optimal jdeodorant.eclipse.commandline.diff.
   * @param deadline Time when the jdeodorant.eclipse.commandline.diff should be complete by.  Used
  ...
  private LinkedList<Diff> diff_main(String text1, String text2,
    ...
    // Compute the jdeodorant.eclipse.commandline.diff on the middle block.
   ...
   *     line-level jdeodorant.eclipse.commandline.diff first to identify the changed areas.
   *     If true, then run a faster slightly less optimal jdeodorant.eclipse.commandline.diff.
   * @param deadline Time when the jdeodorant.eclipse.commandline.diff should be complete by.
   * @return Linked List of Diff objects.
   */
  ...
  /**
   * Do a quick line-level jdeodorant.eclipse.commandline.diff on both strings, then rediff the parts for
   * greater accuracy.
  ...
  private LinkedList<Diff> diff_lineMode(String text1, String text2,
    ...
    // Convert the jdeodorant.eclipse.commandline.diff back to original text.
  ...
  /**
   * Find the 'middle snake' of a jdeodorant.eclipse.commandline.diff, split the problem in two
   * and return the recursively constructed jdeodorant.eclipse.commandline.diff.
  ...
  /**
   * Given the location of the 'middle snake', split the jdeodorant.eclipse.commandline.diff in two parts
   * and recurse.
   * @param text1 Old string to be diffed.
   * @param text2 New string to be diffed.
  ...
  /**
   * Rehydrate the text in a jdeodorant.eclipse.commandline.diff from a string of line hashes to real lines of
   * text.
  ...
  protected String[] diff_halfMatch(String text1, String text2) {
    if (Diff_Timeout <= 0) {
      // Don't risk returning a non-optimal jdeodorant.eclipse.commandline.diff if we have unlimited time.
      return null;
    }
    String longtext = text1.length() > text2.length() ? text1 : text2;
    String shorttext = text1.length() > text2.length() ? text2 : text1;
  ...
  public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
    ...
    // Normalize the jdeodorant.eclipse.commandline.diff.
    if (changes) {
      diff_cleanupMerge(diffs);
    }
    ...
    while (nextDiff != null) {
      if (prevDiff.operation == Operation.EQUAL &&
        ...
        if (!prevDiff.text.equals(bestEquality1)) {
          // We have an improvement, save it back to the jdeodorant.eclipse.commandline.diff.
        ...
        if (lastequality != null
          ...
          if (pre_ins && pre_del) {
            ...
            if (equalities.empty()) {
              ...
              // walk back to the last known safe jdeodorant.eclipse.commandline.diff.
              thisDiff = safeDiff;
            } else {
  ...
  public void diff_cleanupMerge(LinkedList<Diff> diffs) {
            ...
            if (commonlength != 0) {
              if (pointer.hasPrevious()) {
                ...
                assert thisDiff.operation == Operation.EQUAL
                       : "Previous jdeodorant.eclipse.commandline.diff should have been an equality.";
    ...
    // If shifts were made, the jdeodorant.eclipse.commandline.diff needs reordering and another shift sweep.
    if (changes) {
      diff_cleanupMerge(diffs);
    }
  }
  ...
  /**
   * Crush the jdeodorant.eclipse.commandline.diff into an encoded string which describes the operations
   ...
   * operations required to transform text1 into text2, compute the full jdeodorant.eclipse.commandline.diff.
   * @param text1 Source string for the jdeodorant.eclipse.commandline.diff.
  ...
  public LinkedList<Diff> diff_fromDelta(String text1, String delta)
    ...
    for (String token : tokens) {
      ...
      default:
        ...
        throw new IllegalArgumentException(
            "Invalid jdeodorant.eclipse.commandline.diff operation in diff_fromDelta: " + token.charAt(0));
      }
    }
      ...
      case EQUAL:
        ...
        if (aDiff.text.length() >= 2 * Patch_Margin) {
          ...
          if (!patch.diffs.isEmpty()) {
            ...
            // http://code.google.com/p/google-jdeodorant.eclipse.commandline.diff-match-patch/wiki/Unidiff
    ...
    for (Patch aPatch : patches) {
      ...
      if (start_loc == -1) {
        ...
        if (text1.equals(text2)) {
          ...
          // Imperfect match.  Run a jdeodorant.eclipse.commandline.diff to get a framework of equivalent
          // indices.
  ...
  public String patch_addPadding(LinkedList<Patch> patches) {
    ...
    // Add some padding on start of first jdeodorant.eclipse.commandline.diff.
    ...
    // Add some padding on end of last jdeodorant.eclipse.commandline.diff.
  ...
  /**
   * Class representing one jdeodorant.eclipse.commandline.diff operation.
   */
  public static class Diff {
    ...
    /**
     * The text associated with this jdeodorant.eclipse.commandline.diff operation.
     */
    public String text;

    /**
     * Constructor.  Initializes the jdeodorant.eclipse.commandline.diff with the provided values.
     * @param operation One of INSERT, DELETE or EQUAL.
     * @param text The text being applied.
     */
    public Diff(Operation operation, String text) {
      // Construct a jdeodorant.eclipse.commandline.diff with the specified operation and text.
  ...
  public static class Patch {
    ...
    /**
     * Emmulate GNU jdeodorant.eclipse.commandline.diff's format.
     * Header: @@ -382,8 +481,9 @@
     * Indicies are printed as 1-based, not 0-based.
     * @return The GNU jdeodorant.eclipse.commandline.diff string.
     */
    public String toString() {
