 ...
 * http://code.google.com/p/google-ca.concordia.jdeodorant.eclipse.commandline.diff-match-patch/
 *
...
public class TextDiff {
  ...
  public LinkedList<Diff> diff_main(String text1, String text2,
                                    boolean checklines) {
    // Set a deadline by which time the ca.concordia.jdeodorant.eclipse.commandline.diff must be complete.
    long deadline;
    if (Diff_Timeout <= 0) {
  ...
  private LinkedList<Diff> diff_main(String text1, String text2,
    ...
    // Compute the ca.concordia.jdeodorant.eclipse.commandline.diff on the middle block.
  ...
  protected String[] diff_halfMatch(String text1, String text2) {
    if (Diff_Timeout <= 0) {
      // Don't risk returning a non-optimal ca.concordia.jdeodorant.eclipse.commandline.diff if we have unlimited time.
      return null;
    }
    String longtext = text1.length() > text2.length() ? text1 : text2;
    String shorttext = text1.length() > text2.length() ? text2 : text1;
  ...
  public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
    ...
    // Normalize the ca.concordia.jdeodorant.eclipse.commandline.diff.
    if (changes) {
      diff_cleanupMerge(diffs);
    }
    ...
    while (nextDiff != null) {
      if (prevDiff.operation == Operation.EQUAL &&
        ...
        if (!prevDiff.text.equals(bestEquality1)) {
          // We have an improvement, save it back to the ca.concordia.jdeodorant.eclipse.commandline.diff.
        ...
        if (lastequality != null
          ...
          if (pre_ins && pre_del) {
            ...
            if (equalities.empty()) {
              ...
              // walk back to the last known safe ca.concordia.jdeodorant.eclipse.commandline.diff.
              thisDiff = safeDiff;
            } else {
  ...
  public void diff_cleanupMerge(LinkedList<Diff> diffs) {
            ...
            if (commonlength != 0) {
              if (pointer.hasPrevious()) {
                ...
                assert thisDiff.operation == Operation.EQUAL
                       : "Previous ca.concordia.jdeodorant.eclipse.commandline.diff should have been an equality.";
    ...
    // If shifts were made, the ca.concordia.jdeodorant.eclipse.commandline.diff needs reordering and another shift sweep.
    if (changes) {
      diff_cleanupMerge(diffs);
    }
  }
  ...
  public LinkedList<Diff> diff_fromDelta(String text1, String delta)
    ...
    for (String token : tokens) {
      ...
      default:
        ...
        throw new IllegalArgumentException(
            "Invalid ca.concordia.jdeodorant.eclipse.commandline.diff operation in diff_fromDelta: " + token.charAt(0));
      }
    }
      ...
      case EQUAL:
        ...
        if (aDiff.text.length() >= 2 * Patch_Margin) {
          ...
          if (!patch.diffs.isEmpty()) {
            ...
            // http://code.google.com/p/google-ca.concordia.jdeodorant.eclipse.commandline.diff-match-patch/wiki/Unidiff
    ...
    for (Patch aPatch : patches) {
      ...
      if (start_loc == -1) {
        ...
        if (text1.equals(text2)) {
          ...
          // Imperfect match.  Run a ca.concordia.jdeodorant.eclipse.commandline.diff to get a framework of equivalent
          // indices.
  ...
  public String patch_addPadding(LinkedList<Patch> patches) {
    ...
    // Add some padding on start of first ca.concordia.jdeodorant.eclipse.commandline.diff.
    ...
    // Add some padding on end of last ca.concordia.jdeodorant.eclipse.commandline.diff.
  ...
  public static class Diff {
    ...
    public Diff(Operation operation, String text) {
      // Construct a ca.concordia.jdeodorant.eclipse.commandline.diff with the specified operation and text.
